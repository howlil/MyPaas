...
<div class="page-shell py-6">
	...
	{#if adding}
		<SectionPanel title="Add user to whitelist" description="Whitelisted users can authenticate with GitHub OAuth after they are added here." className="mb-5">
			<form class="grid gap-3 md:grid-cols-[minmax(0,1fr)_12rem_auto_auto] md:items-start" on:submit|preventDefault={handleAdd}>
				...
				<ActionButton variant="primary" type="submit" loading={savingUser} loadingLabel="Adding..." disabled={!canAdd}>Add</ActionButton>
				<ActionButton variant="ghost" on:click={() => (adding = false)} disabled={savingUser}>Cancel</ActionButton>
					...
					<tr class="hover:bg-gray-50/80 dark:hover:bg-gray-900/70">
						<td class="px-5 py-4">
							<div class="flex items-center gap-3">
								...
								{:else}
									<div
										class="flex h-8 w-8 items-center justify-center rounded-md border border-gray-200 bg-gray-50 text-xs font-semibold text-gray-500 dark:border-gray-800 dark:bg-gray-900 dark:text-gray-300"
									>
										{initial(user.email)}
									</div>
								{/if}
